...
const SalesVouchers = () => {
  ...
  const fetchData = async () => {
    try {
      const [vouchersRes, customersRes, itemsRes] = await Promise.all([
        axios.get(`${API}/erp/vouchers/sales`),
        axios.get(`${API}/erp/parties?party_type=customer`),
  ...
  const handleOpenDialog = () => {
    const nextVoucherNumber = `INV-${String(vouchers.length + 1).padStart(4, '0')}`;
    setFormData({
  ...
  const addLineItem = () => {
    ...
    setLineItems([...lineItems, {
      ...
      quantity: 1,
      rate: firstItem.sale_rate,
      amount: firstItem.sale_rate,
      tax_rate: firstItem.gst_rate,
      tax_amount: (firstItem.sale_rate * firstItem.gst_rate) / 100,
      total: firstItem.sale_rate + ((firstItem.sale_rate * firstItem.gst_rate) / 100)
    }]);
  };
  ...
  const updateLineItem = (index, field, value) => {
    ...
    if (field === 'item_id') {
      ...
      if (selectedItem) {
        ...
        newItems[index].rate = selectedItem.sale_rate;
  ...
  const handleSubmit = async () => {
    if (!formData.voucher_number || !formData.customer_id || lineItems.length === 0) {
    ...
    try {
      ...
      await axios.post(`${API}/erp/vouchers/sales`, payload);
      ...
      <div className="pt-24 pb-16 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-4xl sm:text-5xl font-bold gradient-text">Sales Invoices</h1>
              <p className="text-gray-600 mt-2">Create and manage sales invoices</p>
            </div>
          ...
          {vouchers.length === 0 ? (
            <Card className="glass text-center py-20">
              <CardContent>
                ...
                <h2 className="text-2xl font-bold mb-4">No invoices yet</h2>
                <Button className="btn-primary" onClick={handleOpenDialog}>
                  <Plus className="w-5 h-5 mr-2" />
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{voucher.customer_name}</p>
          ...
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="voucher_number">Invoice Number *</Label>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <Button onClick={handleSubmit} className="btn-primary flex-1">Create Invoice</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
